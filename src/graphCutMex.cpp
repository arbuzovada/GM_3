    ...
    int nrhs, const mxArray *prhs[])
{
	...
	for(int i = 0; i < numEdges; i++)
	{
		...
		MATLAB_ASSERT(edges[i + 2 * numEdges] >= 0, "graphCutMex: error in pairwise terms array: negative edge weight");
		MATLAB_ASSERT(edges[i + 3 * numEdges] >= 0, "graphCutMex: error in pairwise terms array: negative edge weight");
	}
